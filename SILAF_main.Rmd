...
# The data analysis is based on several result files. Upon publication, the code and associated result files will be available as an R project file.
# Upon request, we provide this file also before, please e-mail florian.schober@ki.se
...
ggplot()+
  ...
  scale_y_continuous(name="Labelling efficiency", breaks = c(0,.2,.4,.6,.8,1))+
  theme_classic()+
  ggtitle("Figure S3A: 14 days labelling efficiency in flies. Mean +/- SD")
dev.off()
...
(1) Why do we not use Arg/Lys double labelling? (2) Is SILAF as accurate as LFQ? The "read.ms" function is described in detailled in the header file ("SILAF_header.R").
...
### Classical SILAF: Figure S4D

```{r Figure S5D, tidy = TRUE, warning = FALSE}
# Read in data >>
...
fit <- eBayes(fit)
limma.result <- topTable(fit,number = Inf,confint = TRUE)
...
fit <- eBayes(fit)
limma.result <- topTable(fit, number = Inf,confint = TRUE, coef = 2)
...
# ... and then import as a formated table to plot a GSEA graph
...
OXPHOS.manual <- ifelse(OXPHOS.manual!=TRUE, FALSE, OXPHOS.manual)
BSF.flies$plots$OXPHOS <- BSF.flies$plots$OXPHOS*as.logical(OXPHOS.manual)
...
## Figure S5E: Fold change overlap
...
phospho.occupancy <- phospho.occupancy[phospho.occupancy$locProb >= 0.75,]
...
string.changed <- as.character(phospho.occupancy[abs(phospho.occupancy$mean) >= 1,]$protein)
string.up <- as.character(phospho.occupancy[phospho.occupancy$mean >= 1,]$protein)
string.down <- as.character(phospho.occupancy[phospho.occupancy$mean <= -1,]$protein)
string.background <- as.character(phospho.occupancy$protein)
write.table(file = "GSEA/Monster/Phosphostring_changed_indv.txt", string.changed, quote = F, col.names = F, row.names = F)
write.table(file = "GSEA/Monster/Phosphostring_up_indv.txt", string.up, quote = F, col.names = F, row.names = F)
write.table(file = "GSEA/Monster/Phosphostring_down_indv.txt", string.down, quote = F, col.names = F, row.names = F)
write.table(file = "GSEA/Monster/Phosphostring_bg_indv.txt", string.background, quote = F, col.names = F, row.names = F)
...
### Exclusion criteria
phospho.occupancy.cuml <- phospho.occupancy.cuml[phospho.occupancy.cuml$PEP<= 0.01,]
...
df <- data.frame(x = c(3,6,9), y = c(3,6,9))
size <- c(6662, 3375, 864)
size <- sqrt(size/pi)*2
